...
fn client_broadcast(mut tx: ResMut<ClientBroadcast>, units: Query<(&Transform, &unit::Unit)>, players: Res<PlayerList>){
    let mut units_vector: Vec<unit::UnitReport> = Vec::new();
    println!("broadcast system");
    units.iter().for_each(|(transform, unit)| {
        let pos = Position{x: transform.translation.x, y: transform.translation.y};
        let new_unit_report = unit::UnitReport{unit_type: unit.unit_type.as_str(), unit_owner: unit.unit_owner.as_str(), position: pos};
        units_vector.push(new_unit_report);
    });

    let mut players_vector: Vec<PlayerReport> = Vec::new();
    players.0.iter().for_each(|player|{players_vector.push(PlayerReport{username: player.username.as_str()})});


    let server_report = ServerReport{players: players_vector, units: units_vector};
    ...
    match report_json {
        Ok(report_json) => {
            ...
            match tx.0.send(report_json){
                Ok(_) => {println!("sent broadcast");},
                Err(error) => {println!("error sending broadcast {}", error);}
            }},
        Err(_) => {println!("could not serialize broadcast message");}
    }
    
}

fn process_event(mut rx: ResMut<EventReceiver>, mut commands: Commands, mut player_list_resource: ResMut<PlayerList>) {
...
fn action_spawn(data: String, username: String, mut commands: Commands){
    ...
    match spawn_data {
        Ok(spawn_data) => {commands.spawn(entities::soldier::new(spawn_data.position.x, spawn_data.position.y, username));},
